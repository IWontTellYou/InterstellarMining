...
public class ShipsShopDialog extends CustomDialog {
    ...
    private void rearangeTable(){
        ...
        for (Ship s : shipShop.keySet()){
            counter--;
            if(counter == 0) {
                getContentTable().add(shipShop.get(s)).row();
                counter = 3;
            }else {
                getContentTable().add(shipShop.get(s));
            }
        }
    ...
    private void createItem(final Ship s){
        ...
        table.add(image)./*.colspan(2).fillX()*/row();
        ...
        TextButton buy = new TextButton("(Buy) " + s.getPrice() + " $", game.getUISkin());
        buy.addListener(new ClickListener(){
            ...
            public void clicked(InputEvent event, float x, float y) {
                if(game.getPlayer().getMoney() >= s.getPrice()){
                    ...
                    getContentTable().removeActor(shipShop.get(s));
        ...
        table.add(buy).fillX().colspan(2).height(DEFAULT_ACTOR_HEIGHT).row();
